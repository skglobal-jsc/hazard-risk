...
export function latLngToTile(lat: number, lng: number, z: number): { tile: TileCoord; pixel: PixelCoord } {
...
export function getPixelFromPNG(png: PNGWithMetadata | undefined, x: number, y: number): [number, number, number] {
  if (!png) return [0, 0, 0];
...
export function calculateElevationFromRGB(r: number, g: number, b: number): number | null {
...
export function createDEMUrlList(demConfigs: any[]): Array<any & { zoom: number }> {
